...
|---|---|---:|---:|---:|
| event_study | ❌ FAIL | 4 | 3 | 1 |
| dcc_garch | ⚠️ WARN | 11 | 3 | 0 |
...
- ❌ `Edit_mj/results/event_study_placebo.csv` — not found
...
- ⚠️ **WARN** — ADF 사전검증 미수행
  - missing patterns: ['adfuller|adf_test|ADF']
- ⚠️ **WARN** — 잔차 진단(Ljung-Box) 미수행
  - missing patterns: ['ljung[_\\s]?box|acorr_ljungbox']
- ✅ **PASS** — 단일 초기값 MLE (지역 최솟값 위험)
  - pattern matched: multi[_\s]?init|n_init|n_starts|grid_search|격자
- ⚠️ **WARN** — seed 미고정 (재현 불가)
  - missing patterns: ['np\\.random\\.seed|random_state\\s*=\\s*[0-9]|RANDOM_SEED']
